...
class BinarySearchTree
  ...
  def delete_reorder(old_node, parent=nil)
    ...
    #if node to be delete has left & right children
    if temp.left && temp.right
      # find min of right tree. swap with node to be deleted
      min = temp.right
      ...
      # if swapped node has right. move up tree to its parent
      if min.right
        swap_parent.right = min.right
      end

      #replace swapped node's children with children of deleted node
      ...
      #if node to be deleted only has left
    elsif temp.left
      if parent
      ...
      #if node to be deleted only has right
    elsif temp.right
      if parent
